...
--3
SELECT P.NOMBRE, P.APELLIDO1, P.APELLIDO2, TRUNC((SYSDATE - P.ANTIGUEDAD)/7, 0) "SEMANAS", NEXT_DAY(SYSDATE, TO_CHAR(P.ANTIGUEDAD, 'DAY')) "PROXIMA SEMANA"
FROM PROFESORES P, DEPARTAMENTOS D
...
--9 
SELECT A1.NOMBRE || ' ' || A1.APELLIDO1 || ' ' || A1.APELLIDO2 "Ella", A2.NOMBRE || ' ' || A2.APELLIDO1 || ' ' || A2.APELLIDO2 "El"
FROM ALUMNOS A1, ALUMNOS A2, MATRICULAR M1, MATRICULAR M2
WHERE UPPER(A1.GENERO) = 'FEM' AND UPPER(A2.GENERO) = 'MASC' 
    AND A1.DNI = M1.ALUMNO AND M1.ASIGNATURA = 112
    AND A2.DNI = M2.ALUMNO AND M2.ASIGNATURA = 112
    AND DECODE(M1.CALIFICACION, 'MH', 10, 'SB', 9, 'NT', 7, 'AP', 5, 'SP', 3, 'NP', 0, 0) > DECODE(M2.CALIFICACION, 'MH', 10, 'SB', 9, 'NT', 7, 'AP', 5, 'SP', 3, 'NP', 0, 0)
    AND ABS(A1.FECHA_PRIM_MATRICULA - A2.FECHA_PRIM_MATRICULA) < 7;
...
--21
SELECT ASIG.NOMBRE, P.ID
FROM ASIGNATURAS ASIG LEFT OUTER JOIN 
    (IMPARTIR I LEFT OUTER JOIN PROFESORES P ON (I.PROFESOR = P.ID)) ON(ASIG.CODIGO = I.ASIGNATURA)
WHERE ASIG.CREDITOS != (NVL(ASIG.PRACTICOS, 0) + NVL(ASIG.TEORICOS, 0));
...
--24
SELECT A1.NOMBRE || ' ' || A1.APELLIDO1 || ' ' || A1.APELLIDO2 "ALUMNO1", A2.NOMBRE || ' ' || A2.APELLIDO1 || ' ' || A2.APELLIDO2 "ALUMNO2"
